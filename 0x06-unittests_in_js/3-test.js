...
describe('Utils.calculateNumber', () => {
  ...
  describe('when type is DIVIDE', () => {
    ...
    it('it should return Error if b is equal to 0', () => {
      assert.equal(
        Utils.calculateNumber('DIVIDE', 10.3, 0).toLowerCase(),
        'error'
      );
      assert.equal(
        Utils.calculateNumber('DIVIDE', 10.7, 0).toLowerCase(),
        'error'
      );
      assert.equal(
        Utils.calculateNumber('DIVIDE', 10.3, 0.3).toLowerCase(),
        'error'
      );
      assert.equal(
        Utils.calculateNumber('DIVIDE', 10.7, 0.2).toLowerCase(),
        'error'
      );
    });
  });
